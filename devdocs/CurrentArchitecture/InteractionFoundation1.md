...
`ProximityPrompt` is a constructible schema-backed `Instance`. It has five
...
property. `RequiresLineOfSight` is deliberately absent rather than exposed as
a no-op: the current physics boundary has no general semantic raycast.
...
does not add a speculative priority property.
...
On every press or hold completion, Engine re-resolves the Player, character,
prompt, anchor, enabled state, finite distance, and exact range before firing.
...
Only the five authored prompt properties persist. Candidate identity, player
...
marks `Engine.ProximityPrompt` constructible and exposes all five fields as
...
far, and disabled distributions; and 1, 4, and 16 synthetic query origins. It
reports registration, a 128-record dirty property batch, query/filter/selection,
and native hold/presentation updates. These local 2026-08-25 results are
regression evidence, not an SLA or an expected 10K-game workload:

| Scenario | Registration | 16-player query P50 / P95 / P99 | Mean considered/player |
| --- | ---: | ---: | ---: |
| 1 dense | 0.06 ms | 0.083 / 0.084 / 0.087 ms | 1 |
| 100 dense | 10.33 ms | 0.098 / 0.099 / 0.101 ms | 100 |
| 1K dense | 349.25 ms | 0.259 / 0.297 / 0.611 ms | 1,000 |
| 10K dense stress | 5,749.90 ms | 3.546 / 5.213 / 6.152 ms | 10,000 |
| 10K sparse | 4,820.96 ms | 0.116 / 0.151 / 0.162 ms | 16 |
| 10K far | 5,194.51 ms | 0.095 / 0.134 / 0.154 ms | 0 |
| 10K disabled | 4,830.84 ms | 0.111 / 0.127 / 0.133 ms | 0 |

Dense 10K is intentionally the worst bounded case because every prompt occupies
the queried cells. Its hold/presentation update P99 was 0.259 ms. Sparse query
...
Deferred work includes a semantic physics raycast and working LOS property,
custom/rebindable/device-aware presentation, server/client request replication,
