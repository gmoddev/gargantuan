...
namespace gargantuan {
	struct InteractionServiceTestAccess {
		...
		static std::pair<ObjectId, std::size_t> Query(InteractionService &Service, const glm::vec3 &Origin) {
			Service.ProcessDirtyPrompts();
			auto Result = Service.QueryNearest(Origin);
			return {Result.Prompt, Result.Considered};
		}
...
namespace {
	...
	glm::vec3 PositionFor(std::string_view Distribution, std::size_t Index) {
		...
		const float X = static_cast<float>(Index % 100) * 24.0f;
		const float Z = static_cast<float>(Index / 100) * 24.0f;
		return {X, 0.0f, Z};
	}

	Json RunScenario(std::size_t Count, std::string_view Distribution, bool Quick) {
		...
		for (const std::size_t PlayerCount : {1u, 4u, 16u}) {
			...
			for (int Iteration = 0; Iteration < Iterations; ++Iteration) {
				...
				for (std::size_t PlayerIndex = 0; PlayerIndex < PlayerCount; ++PlayerIndex) {
					auto [PromptId, QueryConsidered] = InteractionServiceTestAccess::Query(
						*Runtime.Interaction, {static_cast<float>(PlayerIndex) * 0.1f, 0.0f, 0.0f}
					);
					(void)PromptId;
...
int main(int ArgumentCount, char **Arguments) {
	try {
		...
		for (const auto Count : Counts)
			for (const auto Distribution : {"sparse", "dense", "far", "disabled"})
				Results.push_back(RunScenario(Count, Distribution, Quick));
