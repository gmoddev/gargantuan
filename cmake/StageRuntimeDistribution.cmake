...
function(GargantuanStageRuntimeFile Source RelativePath Category)
	...
	if(GARGANTUAN_RUNTIME_CATEGORY_SET)
		file(SHA256 "${Source}" GARGANTUAN_DUPLICATE_SOURCE_DIGEST)
		file(SHA256 "${GARGANTUAN_RUNTIME_DISTRIBUTION_ROOT}/${RelativePath}"
			GARGANTUAN_DUPLICATE_DESTINATION_DIGEST)
		get_property(GARGANTUAN_DUPLICATE_CATEGORY GLOBAL
			PROPERTY "GARGANTUAN_RUNTIME_CATEGORY_${GARGANTUAN_RUNTIME_PATH_KEY}")
		if(NOT GARGANTUAN_DUPLICATE_SOURCE_DIGEST STREQUAL GARGANTUAN_DUPLICATE_DESTINATION_DIGEST OR
			NOT GARGANTUAN_DUPLICATE_CATEGORY STREQUAL Category)
			message(FATAL_ERROR "Conflicting runtime distribution path: ${RelativePath}")
		endif()
		return()
	endif()
...
if(DEFINED GARGANTUAN_OPTIONAL_RUNTIME_FILES AND
	NOT "${GARGANTUAN_OPTIONAL_RUNTIME_FILES}" STREQUAL "")
	string(REPLACE "|" ";" GARGANTUAN_OPTIONAL_RUNTIME_FILE_LIST
		"${GARGANTUAN_OPTIONAL_RUNTIME_FILES}")
	foreach(GARGANTUAN_OPTIONAL_RUNTIME_FILE IN LISTS GARGANTUAN_OPTIONAL_RUNTIME_FILE_LIST)
		get_filename_component(GARGANTUAN_OPTIONAL_RUNTIME_NAME "${GARGANTUAN_OPTIONAL_RUNTIME_FILE}" NAME)
		GargantuanStageRuntimeFile(
			"${GARGANTUAN_OPTIONAL_RUNTIME_FILE}"
			"${GARGANTUAN_OPTIONAL_RUNTIME_NAME}"
			Runtime
		)
	endforeach()
endif()
if(DEFINED GARGANTUAN_NODE_REQUIRED_RUNTIME_COUNT AND
	GARGANTUAN_NODE_REQUIRED_RUNTIME_COUNT GREATER 0)
	math(EXPR GARGANTUAN_NODE_REQUIRED_RUNTIME_LAST
		"${GARGANTUAN_NODE_REQUIRED_RUNTIME_COUNT} - 1")
	foreach(GARGANTUAN_NODE_REQUIRED_RUNTIME_INDEX RANGE 0 ${GARGANTUAN_NODE_REQUIRED_RUNTIME_LAST})
		set(GARGANTUAN_NODE_REQUIRED_RUNTIME_FILE
			"${GARGANTUAN_NODE_REQUIRED_RUNTIME_FILE_${GARGANTUAN_NODE_REQUIRED_RUNTIME_INDEX}}")
		get_filename_component(GARGANTUAN_NODE_REQUIRED_RUNTIME_NAME
			"${GARGANTUAN_NODE_REQUIRED_RUNTIME_FILE}" NAME)
		GargantuanStageRuntimeFile(
			"${GARGANTUAN_NODE_REQUIRED_RUNTIME_FILE}"
			"${GARGANTUAN_NODE_REQUIRED_RUNTIME_NAME}"
			Runtime
		)
	endforeach()
endif()
...
if(DEFINED GARGANTUAN_OPTIONAL_NOTICE_COUNT AND GARGANTUAN_OPTIONAL_NOTICE_COUNT GREATER 0)
	math(EXPR GARGANTUAN_OPTIONAL_NOTICE_LAST "${GARGANTUAN_OPTIONAL_NOTICE_COUNT} - 1")
	foreach(GARGANTUAN_OPTIONAL_NOTICE_INDEX RANGE 0 ${GARGANTUAN_OPTIONAL_NOTICE_LAST})
		GargantuanStageRuntimeFile(
			"${GARGANTUAN_OPTIONAL_NOTICE_FILE_${GARGANTUAN_OPTIONAL_NOTICE_INDEX}}"
			"notices/${GARGANTUAN_OPTIONAL_NOTICE_NAME_${GARGANTUAN_OPTIONAL_NOTICE_INDEX}}"
			Notice
		)
	endforeach()
endif()
