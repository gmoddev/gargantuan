...
namespace gargantuan::network {
	namespace {
		bool ValidatePublish(const PublishReplication &Value) {
			...
			if (Value.Properties.size() > MaximumSnapshotPropertiesPerObject ||
				...
				Value.Tags.size() > MaximumTagsPerInstance)
				return false;
			...
			for (const auto &State : Value.Extensions) {
				if (!State.ExtensionSchemaId.IsValid() || State.DefinitionVersion == 0 ||
					...
					!UniqueExtensions.insert(State.ExtensionSchemaId).second)
					return false;
			...
			for (const auto &State : Value.CustomProperties) {
				if (!State.DeclaringClassSchemaId.IsValid() || State.DefinitionVersion == 0 ||
					...
					!UniqueCustomClasses.insert(State.DeclaringClassSchemaId).second)
					return false;
				for (const auto &[Name, PropertyValue] : State.Properties) {
		...
		bool ValidatePreparedPublish(const PreparedPublishReplication &Value) {
			if (!Value.Object.IsValid() || !Value.Template || !Value.Template->Key.World.IsValid() ||
				Value.Template->Key.Object != Value.Object || Value.Template->Publication.Object != Value.Object ||
				Value.NilProperties.Size() > Value.Template->Publication.Properties.size())
				return false;
			std::string_view PreviousName;
			for (std::size_t Index = 0; Index < Value.NilProperties.Size(); ++Index) {
				const auto Name = Value.NilProperties[Index];
				auto Property = std::ranges::find_if(Value.Template->Publication.Properties, [&](const auto &Entry) {
					return Entry.first == Name;
				});
				if (Property == Value.Template->Publication.Properties.end() ||
					!std::holds_alternative<WireObjectReference>(Property->second) ||
					(Index != 0 && !(PreviousName < Name)))
					return false;
				PreviousName = Name;
			}
			return true;
	...
	void ReplicationView::ForgetReplica(ObjectId Object) {
		...
		std::erase_if(LatestStateSequences, [&](const auto &Entry) { return Entry.first.Object == Object; });
	}
			...
			return std::visit(
				[](const auto &Value) {
					...
					if constexpr (std::is_same_v<Type, PublishReplication>) {
						return IsValidPublishReplication(Value);
					} else if constexpr (std::is_same_v<Type, PreparedPublishReplication>) {
						return ValidatePreparedPublish(Value);
					} else if constexpr (std::is_same_v<Type, PropertyReplicationUpdate>) {
						ValidateProtocolString(
							Value.PropertyName, MaximumProtocolIdentifierBytes, "Replication property name"
						);
						...
						return HasSchema == HasVersion && (!HasSchema || (Value.DeclaringClassSchemaId->IsValid() &&
																		  *Value.DefinitionVersion != 0));
					} else if constexpr (std::is_same_v<Type, ExtensionPropertyReplicationUpdate>) {
						ValidateProtocolString(
							Value.PropertyName, MaximumProtocolIdentifierBytes, "Replication extension property name"
						);
						ValidateProtocolWireValue(Value.Value);
	...
	bool IsPublishReplication(const ReplicationIntent &Intent) {
		return std::holds_alternative<PublishReplication>(Intent) ||
			   std::holds_alternative<PreparedPublishReplication>(Intent);
	}

	bool IsValidPublishReplication(const PublishReplication &Publish) noexcept {
		try {
			return ValidatePublish(Publish);
		} catch (...) {
			return false;
		}
	}

	ObjectId GetReplicationObject(const ReplicationIntent &Intent) {
